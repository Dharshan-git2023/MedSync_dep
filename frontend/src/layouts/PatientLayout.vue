    ...
    <aside class="w-60 bg-gradient-to-b from-green-600 to-green-700 text-white shadow-lg flex flex-col">
      ...
      <nav class="mt-6 flex-1 overflow-auto">
        <router-link
          ...
          :key="item.path"
          :to="item.path"
          class="flex items-center px-6 py-3 hover:bg-green-600 transition-colors"
          :class="{ 'bg-green-600 border-l-4 border-lime-400': isActive(item.path) }"
        >
...
import { Bell, Home, FileText, Pill, ShoppingCart } from 'lucide-vue-next'
